...
if (_repNeeded > _currentReputation) then {
    ...
    _textboxInfos ctrlSetStructuredText composeText [ format [localize "str.dpl.equipment.vendor.repneeded", _repNeeded], lineBreak,
                                                      format [localize "str.dpl.equipment.vendor.remain", _repRemain]];
}else {
    _textboxInfos ctrlSetStructuredText parseText format["<t size='2' color='#68b25c'>%1</t>", localize "str.dpl.equipment.vendor.allowed"];
};
