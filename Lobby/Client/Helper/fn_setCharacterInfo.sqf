...
private _composedInfoText = composeText [localize "str.dpl.profile.info.name", " ", _name, lineBreak,
...
if(_state isEqualTo STATE_WIA) then {
    private _parsedText = parseText format["<t size='2' color='#ff0000'>%1</t>", localize "str.dpl.character.state.wia"];
    _composedInfoText = composeText [_parsedText, lineBreak,
                                     localize "str.dpl.profile.info.wia.timeleft", " ", str ceil((_hospilitationTimeLeft / 60)), " min"];
};
