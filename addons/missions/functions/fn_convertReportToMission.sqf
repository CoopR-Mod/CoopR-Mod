...
if (isServer) then {
...

    {
        ...
        private _entry = _x;
        private _owner = [_entry, COOPR_CHAR_RECON_ENTRY_OWNER] call CBA_fnc_hashGet;
        ...
        if (_entryAccuracy > 50) then {
            ...
            [_newCooprMission, COOPR_CHAR_MISSION_TYPE, _cooprMissionType] call CBA_fnc_hashSet;
            [_newCooprMission, COOPR_CHAR_MISSION_DESCRIPTION, ""] call CBA_fnc_hashSet; // inactive
            [_newCooprMission, COOPR_CHAR_MISSION_REPORT_TIME, _time] call CBA_fnc_hashSet;
            [_newCooprMission, COOPR_CHAR_MISSION_MARKER, _serializedMarkers] call CBA_fnc_hashSet;
            [_newCooprMission, COOPR_CHAR_MISSION_ACCURACY, _entryAccuracy] call CBA_fnc_hashSet;
            [_newCooprMission, COOPR_CHAR_MISSION_TARGET, [_type, _strength]] call CBA_fnc_hashSet;
    ...
    private _missionTracker = _unit getVariable [COOPR_CHAR_MISSION_TRACKER, []];
    if (_missionTracker isEqualTo []) then {
        ...
        if ((_infStrengths isEqualTo []) and (_motorizedStrengths isEqualTo []) and (_armoredStrengths isEqualTo [])) then {
            [_missionTracker, COOPR_CHAR_MISSION_TRACKER_RECON_COMPLETE, true] call CBA_fnc_hashSet;
