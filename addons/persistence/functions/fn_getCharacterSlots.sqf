...
INFO2("retrieving character slots for player %1", _playerUid);
if (_hasNoProfiles) then {
    INFO("no player profiles found - skipping fetch");
};

if(_hasPlayerEntry) then {
    _slots = [_allProfiles, _playerUid] call CBA_fnc_hashGet;
    INFO2("found character slots for id %1", _playerUid);
    //{ [_x] call coopr_fnc_characterStatePrettyLog; } forEach _slots;
} else {
    INFO2("failed to get character slots - no entry for player uid %1 was found", _playerUid);
};
