 ...
 * Author: xetra11
 *
 * Creates a CoopR assault task that is applied to the whole group of a given unit
 *
 * Arguments:
 ...
 *
 * Example:
 * [_unit, _cooprTaskInfo] call coopr_fnc_createAssaultTask;
 *
 * Public: No
 *
 * Scope: Server
...
if (isServer) then {
    DEBUG2("assigning assault team task to unit %1", _unit);
    ...
    private _taskName = format ["coopr_task_assault_%1", [COOPR_COUNTER_TASKS, COOPR_TASK_TYPE_ASSAULT] call CBA_fnc_hashGet];
    private _creationSuccess = [_unit, _taskName , [_description, "Assault"], _location, 1, 2, true] call BIS_fnc_taskCreate;
    ...
    if (_creationSuccess isEqualTo true) then {
        ...
        DEBUG("assault task assigned");
        [COOPR_TASK_TYPE_ASSAULT] remoteExec ["coopr_fnc_countTask"];
