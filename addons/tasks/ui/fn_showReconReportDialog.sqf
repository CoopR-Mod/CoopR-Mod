...
//promise for report id
[[_characterID], "coopr_fnc_getReportIdForCharacter", [], {
    params ["_callbackArgs", "_promisedResult"];
    private _reportID = _promisedResult;
    DEBUG2("recon report id is: %1", _reportID);

    // promise for recon entries
    [[_reportID], "coopr_fnc_getEntriesForReport", [], {
        params ["_callbackArgs", "_promisedResult"];
